 ...
 *
 */
public final class Bee2XMLSignature extends DOMStructure
    implements XMLSignature {

    private static final Logger LOG = System.getLogger(Bee2XMLSignature.class.getName());
     ...
     */
    public Bee2XMLSignature(SignedInfo si, KeyInfo ki,
     ...
     */
    public Bee2XMLSignature(Element sigElem, XMLCryptoContext context,
                           Provider provider)
        throws MarshalException
    {
        ...
        Element siElem = DOMUtils.getFirstChildElement(localSigElem,
                                                       "SignedInfo",
                                                       XMLSignature.XMLNS);
        si = new Bee2SignedInfo(siElem, context, provider);
        ...
        throws MarshalException
    {
        ...
        ((Bee2SignedInfo)si).marshal(sigElem, dsPrefix, context);
        ...
        throws MarshalException, XMLSignatureException
    {
        ...
        for (Reference ref : allReferences) {
            digestReference((Bee2Reference)ref, signContext);
        }

        // do final sweep to digest any references that were skipped or missed
        for (Reference ref : allReferences) {
            if (((Bee2Reference)ref).isDigested()) {
                continue;
            }
            ((Bee2Reference)ref).digest(signContext);
        }
        ...
        try {
            byte[] val = ((AbstractBee2SignatureMethod)
    ...
    private void digestReference(Bee2Reference ref, XMLSignContext signContext)
        throws XMLSignatureException
    {
        if (ref.isDigested()) {
            return;
        ...
        if (Utils.sameDocumentURI(uri)) {
            ...
            if (parsedId != null && signatureIdMap.containsKey(parsedId)) {
                ...
                if (xs instanceof Bee2Reference) {
                    digestReference((Bee2Reference) xs, signContext);
                } else if (xs instanceof Manifest) {
                    Manifest man = (Manifest) xs;
                    List<Reference> manRefs = DOMManifest.getManifestReferences(man);
                    for (Reference manRef : manRefs) {
                        digestReference((Bee2Reference) manRef, signContext);
                    }
                }
            }
        ...
        implements SignatureValue
    {
            ...
            throws XMLSignatureException
        {
            ...
            try {
                validationStatus = ((AbstractBee2SignatureMethod)sm).verify
